...
exifAttrs = set(["Model", "Make", "ExifImageWidth", "ExifImageHeight", "FocalLength", "ImageWidth", "ImageLength"])
...
class OsmBundler():
        ...
        if 'Make' in exif and 'Model' in exif:
            ...
            if ccdWidth:
                if 'FocalLength' in exif and 'ExifImageWidth' in exif and 'ExifImageHeight' in exif:
                    ...
                    width = float(exif['ExifImageWidth']) if exif['ExifImageWidth'] != 0 else exif['ImageWidth']
                    height = float(exif['ExifImageHeight']) if exif['ExifImageHeight'] != 0 else exif['ImageLength']
    ...
    def extractFeatures(self, photo):
        ...
        logging.info("\nFeatures extracted to => %s.%s" % (photo, self.featureExtractor.fileExtension))
